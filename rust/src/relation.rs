            ...
            binary!(Ge | Gt, var!(x), constant!(a)) | binary!(Le | Lt, constant!(a), var!(x)) => {
                if let Some(r) = get_mut(self, x) {
                    let inf = a
                        .interval()
                        .iter()
                        .fold(f64::NEG_INFINITY, |acc, x| acc.max(x.x.inf()));
                    *r = r.intersection(interval!(inf, f64::INFINITY).unwrap());
                }
            }
            binary!(Le | Lt, var!(x), constant!(a)) | binary!(Ge | Gt, constant!(a), var!(x)) => {
                if let Some(r) = get_mut(self, x) {
                    let sup = a
                        .interval()
                        .iter()
                        .fold(f64::INFINITY, |acc, x| acc.min(x.x.sup()));
                    *r = r.intersection(interval!(-f64::INFINITY, sup).unwrap());
                }
            }
            _ => (),
        }
